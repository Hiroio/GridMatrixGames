...
final class StatsViewModel {
  ...
  private func row(for game: GameType) -> StatsActivityRow {
    ...
    return StatsActivityRow(
      ...
      title: game.title,
      icon: game.icon,
      metricLabel: "Fewest moves",
      bestText: bestGameText(results: results),
      lastText: lastGameText(results: results),
      sessions: results.count,
  ...
  private func bestText(for test: TestType, results: [TrainingResult]) -> String {
    ...
    switch test {
    case .gridSearch:
      guard let ms = results.map(\.primaryMetric).min() else { return "—" }
  ...
  private func bestGameText(results: [TrainingResult]) -> String {
    guard let moves = results.map(\.primaryMetric).min() else { return "—" }
    return "\(Int(moves)) moves"
  }

  private func lastGameText(results: [TrainingResult]) -> String {
    guard let last = results.first else { return "—" }
